...
static void GenerateTestImageICC(base::FilePath *output_path) {
  ...
  ASSERT_EQ(cjpeg(9, command_line), 0);
}

TEST(JPEGTranTest, ICC) {

  base::FilePath input_image_path(FILE_PATH_LITERAL(GetTargetDirectory()));
  input_image_path = input_image_path.AppendASCII("testout_rgb_islow.jpg");
  GenerateTestImageICC(&input_image_path);
  ASSERT_TRUE(base::PathExists(input_image_path));
