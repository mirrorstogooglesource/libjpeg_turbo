                                    ...
                                    JSAMPARRAY *output_data_ptr)
{
  ...
  for (int inrow = 0; inrow < max_v_samp_factor; inrow++) {
    ...
    *outptr = (JSAMPLE)GETJSAMPLE(*inptr);
    ...
    /* Initially 1 - due having already stored the first pixel of the image. */
    /* However, in subsequent iterations of the SIMD loop this offset is */
    /* (2 * colctr - 1) to stay within the bounds of the sample buffers */
