...
/*
 * The diagram below shows a grid-window of chroma samples produced by
 * h2v2 downsampling.
 *
 *                  s0        s1
 *             +---------+---------+
 ...
 *             +---------+---------+
 *
 * Every sample contains four of the original pixel chroma values. The pixels'
 * chroma values are centred at positions p0, p1, p2,..., p23 above. For a
 * given grid-window position, r1 is always used to denote the row of samples
 * containing the pixel chroma values we are computing. For the top row of
 * pixel chroma values in r1 (p8-p11), the nearest neighbouring samples are in
 * the row above - denoted by r0. Likewise, for the bottom row of pixels in r1
 * (p8-p11), the nearest neighbouring samples are in the row below - denoted by
 * r2.
 *
 * To compute the pixel chroma values of the original image, we proportionally
 ...
 *    Pixel chroma value p8 only contains components from sample column s0. Its
 *    value is computed by blending samples s0r1 and s0r0 in the ratio 3:1.
 * 2) The last pixel in this row of the original image.
 *    Pixel chroma value p11 only contains components from sample column s1. Its
 *    value is computed by blending samples s1r1 and s1r0 in the ratio 3:1.
 * 3) General case (all other pixels in the row).
 *    Apart from the first and last pixels, every other pixel chroma value in
 ...
 *
 * The bottom row of pixel chroma values in row r1 can be computed in the same
                                    ...
                                    JSAMPARRAY *output_data_ptr)
{
  ...
  while (outrow < max_v_samp_factor) {
    ...
    /* Case 1: first pixel chroma value in this row of original image. */
    ...
    *outptr1 = (JSAMPLE)((s0colsum1 * 4 + 8) >> 4);
    ...
    /* Store pixel chroma values to memory. */
    ...
    /* Case 2: last pixel chroma value in this row of the original image. */
